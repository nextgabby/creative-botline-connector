...
Slack listener bot that watches `#x-creative-hotline` for new hotline submissions
(posted by the existing Workflow) and auto-generates 5-7 creative concepts using Grok,
drawing on 12 months of hotline history for context.

## How it works

```
Workflow posts in #x-creative-hotline
        |
        v
Bot detects message (contains "Submit a Hotline Request" or SS-/CR- pattern)
        |
        v
...
Downloads attached images (vision) + documents/decks (text extraction)
        |
        v
Paginates back 12 months of #x-creative-hotline history via conversations.history
...
Calls Grok (grok-4.3) with:
  ...
  - Hotline intelligence (up to 15 past submissions + strategist replies)
...
startup and sent verbatim as the Grok system message. The bot passes hotline
...
- **Detection**: Regex for "Submit a Hotline Request" or `XX-NNNNN` Request ID patterns
...
- **History**: Paginates `conversations.history` back 12 months (up to 10 pages of 200 messages), filters for hotline submissions, fetches thread replies via `conversations.replies`, caches results for 30 minutes. Up to 15 past examples are sent to Grok as "Hotline Intelligence."
